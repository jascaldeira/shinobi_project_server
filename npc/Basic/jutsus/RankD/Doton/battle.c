...
#define IF_CASE(__type, __find) \
	do { \
		if((if_)->type == (__type) && !found) { \
			flag = (int)(status_get_ ## __find(src) - status_get_ ## __find(bl)); \
			found = true; \
		} \
	} while(0);
 ...
 * Compare two values between targets
 *------------------------------------------*/
int battle_calc_difference(struct block_list* src, struct block_list* bl, struct if_compare* if_)
{
	int flag;
	bool found;
	...
	IF_CASE(	SP_SPEED,		speed	);
	IF_CASE(	SP_HP,			hp		);
	IF_CASE(	SP_MAXHP,		max_hp	);
	IF_CASE(	SP_SP,			sp		);
	IF_CASE(	SP_MAXSP,		max_sp	);
	IF_CASE(	SP_BASELEVEL,	lv		);
	IF_CASE(	SP_STR,			str		);
	IF_CASE(	SP_AGI,			agi		);
	IF_CASE(	SP_VIT,			vit		);
	IF_CASE(	SP_INT,			int		);
	...
	IF_CASE(	SP_LUK,			luk		);
	IF_CASE(	SP_DEF1,		def		);
	IF_CASE(	SP_DEF2,		def2	);
	IF_CASE(	SP_MDEF1,		mdef	);
	IF_CASE(	SP_MDEF2,		mdef2	);
	IF_CASE(	SP_HIT,			hit		);
	IF_CASE(	SP_FLEE1,		flee	);
	IF_CASE(	SP_FLEE2,		flee2	);
	IF_CASE(	SP_ASPD,		amotion	);
	IF_CASE(	SP_CRITICAL,	critical	);
	IF_CASE(	SP_MATK1,		matk_min	);
	IF_CASE(	SP_MATK2,		matk_max	);
	...
	if (!found)
	{
		...
		// Special case (SP_USTR-SP_ULUK)
		// For players, compares base STR-LUK
		// For monsters, compares STR-LUK
		switch (if_->type)
		{
			case SP_USTR:
				flag = sd->base_status.str - (tsd ? tsd->base_status.str : status_get_str(bl));
			case SP_UAGI:
				if (!flag)	flag = sd->base_status.agi - (tsd ? tsd->base_status.agi : status_get_agi(bl));
			case SP_UVIT:
				if (!flag)	flag = sd->base_status.vit - (tsd ? tsd->base_status.vit : status_get_vit(bl));
			case SP_UINT:
				if (!flag)	flag = sd->base_status.int_ - (tsd ? tsd->base_status.int_ : status_get_int(bl));
			case SP_UDEX:
				if (!flag)	flag = sd->base_status.dex - (tsd ? tsd->base_status.dex : status_get_dex(bl));
			case SP_ULUK:
				if (!flag)	flag = sd->base_status.luk - (tsd ? tsd->base_status.luk : status_get_luk(bl));
			// If we found any one of them:
				found = true;
				break;
			default:
				;
		...
		if (!found)
		{
			...
			{
				case SP_JOBLEVEL:
					flag = sd->status.job_level - tsd->status.job_level;
					break;
				case SP_BASEEXP:
					flag = sd->status.base_exp - tsd->status.base_exp;
					break;
				case SP_JOBEXP:
					flag = sd->status.job_exp - tsd->status.job_exp;
					break;
				case SP_WEIGHT:
					flag = sd->weight - tsd->weight;
					break;
				case SP_MAXWEIGHT:
					flag = sd->max_weight - tsd->max_weight;
					break;
				case SP_ATK1:
					flag = (sd->battle_status.rhw.atk + sd->battle_status.lhw.atk) - (tsd->battle_status.rhw.atk + sd->battle_status.lhw.atk);
					break;
				case SP_ATK2:
					flag = (sd->battle_status.rhw.atk2 + sd->battle_status.lhw.atk2) - (tsd->battle_status.rhw.atk2 + sd->battle_status.lhw.atk2);
					break;
				case SP_ZENY:
					flag = sd->status.zeny - tsd->status.zeny;
					break;
			}
		}
	}
...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv)
{
	...
	if (ssd)
	{
		...
		if (ssd->if_state[0].type > 0)
		{
			for (i = 0; MAX_PC_BONUS > i; i++)
			{
				int mod, mod2;

				if (ssd->if_state[i].type == 0)
					continue;

				// Do a check for particular values
				// These are values which we can't read from mobs
				if (!(bl->type == BL_PC))
					switch (ssd->if_state[i].type) {
					case SP_STATUSPOINT:	case SP_SKILLPOINT:		case SP_ZENY:
					case SP_SEX:		case SP_NEXTBASEEXP:	case SP_NEXTJOBEXP:
					case SP_WEIGHT:		case SP_MAXWEIGHT:		case SP_USTR:
					case SP_UAGI:		case SP_UVIT:		case SP_UINT:
					case SP_UDEX:		case SP_ULUK:		case SP_JOBLEVEL:
						continue;
					}

				mod = battle_calc_difference(src,bl,&ssd->if_state[i]);

				if (!((ssd->if_state[i].flag&IF_MORE && mod > 0) ||
					  (ssd->if_state[i].flag&IF_EQUAL && mod == 0) ||
					  (ssd->if_state[i].flag&IF_LESS && mod < 0)))
					continue;

				mod2 = ssd->if_state[i].modifier;

				if ((ssd->if_state[i].flag&IF_MAKEPOS && mod < 0) ||
					(ssd->if_state[i].flag&IF_MAKENEG && mod > 0));
					mod *= -1;

				if (ssd->if_state[i].flag&IF_TRUEFIXED)
					damage += mod2;
				if (ssd->if_state[i].flag&IF_TRUEPERCENT)
					damage += damage * mod2/100;
				if (ssd->if_state[i].flag&IF_FIXED)
					damage += (mod * mod2);
				if (ssd->if_state[i].flag&IF_PERCENT)
					damage += damage * (mod * mod2)/1000;
			}
		}
	}
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		{	//Calc base damage according to skill
			case NJ_ISSEN:
				...
				break;
			case PA_SACRIFICE:
				wd.damage = sstatus->max_hp* 7/100;
				wd.damage2 = 0;
				break;
			case LK_SPIRALPIERCE:
	...
	if( tsd && !(nk&NK_NO_CARDFIX_DEF) )
	{
		...
		if( !(nk&NK_NO_ELEFIX) )
		{
			int mod = tsd->subele[s_ele];

			if (wd.flag&tsd->subele_flag[s_ele].flag&BF_WEAPONMASK &&
				wd.flag&tsd->subele_flag[s_ele].flag&BF_RANGEMASK &&
				wd.flag&tsd->subele_flag[s_ele].flag&BF_SKILLMASK)
				mod += tsd->subele_flag[s_ele].amount;

			cardfix=cardfix*(100-mod)/100;

			if( flag.lh && s_ele_ != s_ele )
			{
				mod = tsd->subele[s_ele_];
				
				if (wd.flag&tsd->subele_flag[s_ele_].flag&BF_WEAPONMASK &&
					wd.flag&tsd->subele_flag[s_ele_].flag&BF_RANGEMASK &&
					wd.flag&tsd->subele_flag[s_ele_].flag&BF_SKILLMASK)
					mod += tsd->subele_flag[s_ele_].amount;
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		if( tsd && !(nk&NK_NO_CARDFIX_DEF) )
	  	{ // Target cards.
			...
			if (!(nk&NK_NO_ELEFIX))
			{
				...
				if (ad.flag&tsd->subele_flag[s_ele].flag&BF_WEAPONMASK &&
					ad.flag&tsd->subele_flag[s_ele].flag&BF_RANGEMASK &&
					ad.flag&tsd->subele_flag[s_ele].flag&BF_SKILLMASK)
					mod += tsd->subele_flag[s_ele].amount;
	...
	{
		int rdamage = battle_calc_return_damage(target, ad.damage, ad.flag);
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	{// misc damage reduction from equipment
		...
		if (!(nk&NK_NO_ELEFIX))
		{
			...
			if (md.flag&tsd->subele_flag[s_ele].flag&BF_WEAPONMASK &&
				md.flag&tsd->subele_flag[s_ele].flag&BF_RANGEMASK &&
				md.flag&tsd->subele_flag[s_ele].flag&BF_SKILLMASK)
				mod += tsd->subele_flag[s_ele].amount;
...
// Epoque's Expansion Pack
//Calculates BF_WEAPON 'and BF_MAGIC' returned damage.
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, unsigned int tick, int flag)
{
	...
	if (sc)
	{
		if (sc->data[SC_SACRIFICE])
		{
			...
			status_zap(src, sstatus->max_hp*7/100, 0);//Damage to self is always 9%
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ // Checks on actual target
		case BL_PC:
			...
			if (src->type == BL_MOB && strlen(((TBL_MOB*)src)->db->mf.str) && atoi(((TBL_MOB*)src)->db->mf.value) == pc_readglobalreg((TBL_PC*)target, ((TBL_MOB*)src)->db->mf.str))
				return -1; // Player is friend of mob
			break;
		case BL_MOB:
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_lv",				&battle_config.max_lv,						  99,	 0,	  999,			},
...
// eAthena additions
	...
	// == Weapon level configuration options ==
	{ "wlv_userexp",						&battle_config.wlv_userexp,						5,	  -INT_MAX,	  10000,   },
	{ "wlv_maxlevel",						&battle_config.wlv_maxlevel,					5,	  1,	  99,			 },
	{ "wlv_takeexp",						&battle_config.wlv_takeexp,						0,	  0,	  1,			  },
	{ "wlv_maxexp",							&battle_config.wlv_maxexp,						0,	  0,	  INT_MAX,		},
	{ "wlv_multilevel",						&battle_config.wlv_multilevel,					1,	  0,	  1,			  },
	{ "wlv_gaindefbonus",					&battle_config.wlv_gaindefbonus,				1,	  0,	  1,			  },
...
// BattleGround Settings
	...
	{ "disp_guild_hp",			&battle_config.disp_guild_hp,					0,		0,		7,				},
#ifndef TXT_ONLY
	/**
	 * Achievement System
	 **/
	{ "achievement_exp_rate",			&battle_config.achievement_exp_rate,		100,	1,	INT_MAX,	},
	{ "achievement_cutin_duration",		&battle_config.achievement_cutin_duration,	3000,	1,	INT_MAX,	},
#endif // TXT_ONLY
